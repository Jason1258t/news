...
<important>Ты должен ответить в виде json файла. Другие слова и уточнения не приветствуются. Если ссылки на фотографии не указаны, но вместо них вставь 'nopic'<important>
<important>Обязательные поля: slug, title, description, content, Поле slug должно быть уникальным</important>
<important> Твой ответ будет сериализован в json формат, так что твой ответ должен начинаться с '{' и заканчиваться '}' </important>

<important> Отправленный текст может быть любого формата, как в .md так и в любом другом формате. При составлении json, надо сделать так, чтобы текст выглядел адекватно. без лишних # и других знаков (как в .md например) </important>
example:
{
...
<final_check> Перепроверь соответствует ли json примеру и структуре json'а, а также перепроверь есть ли лишние символы. Если какая то из проверок оказалась неудачной, составь json заново, с учетом всех ошибок`;
